...
{
    /// <summary>
    /// Логика взаимодействия для AddLearnerWindow.xaml
    /// </summary>
    public partial class AddLearnerWindow : Window
    {
        private List<SportSchoolClassLibrary.Section> sections;
        public AddLearnerWindow()
        {
            InitializeComponent();
        ...
        private void AddLearner()
        {
            ...
            if (!validated) return;
            var learner = new SportSchoolClassLibrary.Learner()
            {
                ID = 0,
                SectionID = 1,//GetSectionID(SectionCB.SelectedItem.ToString()),
                Name = NameTB.Text,
            ...
            if (!adminClient.EmailExists(learner.Email))
            {
                adminClient.InsertNewLearner(learner);
            ...
            else
            {
                ...
                WarningLbl.Margin = new Thickness(130, 20 * 14 - 10, 0, 0);
                AnimateWarningLabel();
        ...
        private bool Check()
        {
            ...
            {
                WarningLbl.Content = "← Дата рождения должна быть выбрана";
                WarningLbl.Margin = new Thickness(width, height * 10 - 10, 0, 0);
                AnimateWarningLabel();
                AnimateUIElement("DatePicker");
                return false;
            }
            ...
            {
                WarningLbl.Content = "← Поле не должно быть пустым";
                WarningLbl.Margin = new Thickness(width, height * 16 - 10, 0, 0);
                AnimateWarningLabel();
        ...
        private void AnimateUIElement(string UIElementName, string from = "OrangeRed")
        {
            ...
            switch (UIElementName)
            {
                ...
                case "DatePicker": DatePicker.BorderBrush = brush; break;
        ...
        public static string ModifyToCorrectForm(string text)
        {
